...
/*
 * Os formatadores desta conta vêm de `sugestao-ajustada`, que é o ficheiro
 * SEM dependências de servidor. A dependência aponta deliberadamente para esse
 * lado: o ecrã do profissional chama a conta refeita e não pode arrastar o
 * mysql2 para o browser — ver a nota no fim deste ficheiro.
 */
import { aosCentimos, euros, km, horasPorExtenso } from "@/lib/sugestao-ajustada";
...
/*
 * `sugestaoComOutroTempo` vive em `sugestao-ajustada.ts`, e não aqui.
 *
 * Este ficheiro importa o `pricing-helper`, que importa o `db`, que importa o
 * mysql2 e o drizzle. Enquanto o ecrã do profissional só lhe pedia o TIPO isso
 * não custava nada — um `import type` desaparece na compilação. No instante em
 * que ele passou a CHAMAR a função de refazer a conta, o browser passou a
 * receber a camada de base de dados inteira, e o `next build` recusou-se.
 *
 * A função que refaz a conta não precisa de nada disto: precisa da sugestão já
 * feita e das taxas. Vive num ficheiro sem dependências de servidor, e é DAÍ
 * que vêm os formatadores desta conta — a dependência aponta para o lado
 * seguro, e não há duas maneiras de escrever «2,5 h».
 */
