 ...
 *
 * `service_requests.details` é jsonb: o motor da app grava lá um objeto, não
 * uma frase. Se esse objeto chegar ao JSX como filho direto, o React atira
 ...
 *
 * A regra é sanear na fronteira: nenhuma rota do painel devolve um objeto
 * num campo que a interface vai imprimir. O objeto original, quando é útil,
...
export function safeText(value: unknown): string | null {
  ...
  // Objeto: não há texto honesto a inventar aqui. Quem precisa do conteúdo
  // usa o campo _meta; quem só quer imprimir recebe null e mostra o fallback.
  if (typeof value === "object") return null;
  return String(value);
}

/** O objeto por trás de um campo de texto, quando existe. */
