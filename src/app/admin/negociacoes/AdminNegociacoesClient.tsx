...
const ESTADO_CLS: Record<string, string> = {
  ...
  aguarda_contratacao: "bg-amber-100 text-amber-800",
  acordada: "bg-emerald-100 text-emerald-800",
  desistida: "bg-slate-200 text-slate-600",
  morta: "bg-slate-200 text-slate-500",
};

export default function AdminNegociacoesClient() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <header className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Pedidos da plataforma</h1>
          <p className="mt-1 text-sm text-slate-500">
            {pedidos.length} pedidos com valores. Reenviar emite um link novo e{" "}
            <strong>invalida o anterior</strong>.
          </p>
        </div>
        <button
          onClick={carregar}
                    ...
                    <div key={n.id}>
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-lg border border-slate-300 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-50"
                        >
                          {ocupado === chave ? (
...
function LinkEmClaro({ caminho, aviso }: { caminho: string; aviso: string }) {
  ...
  return (
    <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2.5">
      <p className="text-xs font-semibold text-amber-900">{aviso}</p>
      <div className="mt-1 flex items-center gap-2">
        <code className="flex-1 overflow-x-auto whitespace-nowrap rounded bg-white px-2 py-1 font-mono text-[11px] text-slate-700">
          {url}
        </code>
        <button
          onClick={() => {
